# AIOCR 交叉核验原型 — 文案对齐清单（已落地）

> **状态**：✅ 决策已确认并已统一改入原型（commit `1a73e53`）。本稿保留作为文案口径记录。
> **用途**：把原型现有文案与《AIOCR 需求文档》(OCR 部分) 的官方措辞逐条对照，得出统一后的最终文案。

## 决策结论（已执行）

| 决策 | 结论 | 落地 |
|------|------|------|
| 1 · Skip with Note | 改为 `Justification` | ✅ |
| 2 · Missing field | 归为 Exist 存在性类型、按钮 `Justification`，保留收入类 `+ Add Item` 补录入口 | ✅ |
| 3 · 列表进入按钮 | 对齐 `Review` | ✅ |
| 4 · OCR 状态 `OCR Succeeded`/`Manual Override` | 🔧 待后端确认命名（非用户可见文案，暂不改）| 待后端 |
| 5 · `Review_Required` 字段格式校验态 | 已在《告警与字段状态定义》§2.1 定义；待真实正则/格式校验数据接入后在原型呈现 | 已定义 |
